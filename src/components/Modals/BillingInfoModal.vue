  ...
  <div class="absolute inset-0 flex items-center justify-center overflow-y-auto">
           ...
           <!-- Shared Wrapper (Optional) -->
            ...
            <div class="max-w-5xl mx-auto bg-white rounded-xl shadow p-6 mt-6 space-y-4">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button
                    class="flex-none px-4 py-2 my-2 rounded-lg duration-300 text-sm bg-blue-500 text-white"
                    @click="getSummaryTrainees"
                    >
                ...
                <div class="col-span-2">
                    <div class="font-semibold text-gray-600">Trainees: ({{ student_rows_list.length }})</div>
                    <div class="mt-6 overflow-y-auto max-h-80">
                        ...
                        <table class="min-w-full border-collapse text-xs">
                            ...
                            <tbody>
                            <tr
                                v-for="(row, index) in student_rows_list"
                                :key="index"
                            ...
                            >
                                ...
                                <p class="w-full px-2 py-1">{{ row.fname }}</p>
                                </td>

                                <td class="border px-4 py-2">
                                <p class="w-full px-2 py-1">{{ row.lname }}</p>
                                </td>

                                <td class="border px-4 py-2">
                                <p class="w-full px-2 py-1">{{ row.extension }}</p>
                                </td>

                                <td class="border px-4 py-2">
                                <p class="w-full px-2 py-1">{{ row.mname }}</p>
                                </td>

                                <td class="border px-4 py-2">
                                <p class="w-full px-2 py-1">{{ row.sg_number }}</p>
                                </td>
                       ...
                       <table class="min-w-full border-collapse text-xs">
                        ...
                        <tbody>
                            ...
                            <td class="border px-4 py-2">
                                ...
                                <option v-if="billing_data.bt_id.description != 'Assessment Cost'" value="active">Active</option>
                                ...
                                <option v-if="billing_data.bt_id.description == 'Assessment Cost'" value="absent">Absent</option>
                                <option v-if="billing_data.bt_id.description != 'Assessment Cost'" value="dropped">Dropped</option>
                        ...
                        <!-- Add Row Button -->
                        <div class="mt-2" v-if="student_rows.length < data.num_pax">
                            <button
                                @click="addRow"
                                class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600"
                            >
    ...
    export default{
      ...
      totalUtilized() {
        ...
        canReviewBilling() {
            if (!this.sortedBtypes.length) return false;

            const lastItem = this.sortedBtypes[this.sortedBtypes.length - 1];
    ...
    updateBillingData() {
        ...
        // }
            ...
            getTraineesList(ctrl_num){
                axios.get(process.env.VUE_APP_BASE_URL + '/trainees/get_trainees_list/' + ctrl_num)
                .then((res)=>{
            ...
            findData(){
                ...
                .then((res)=>{
                    ...
                    if(res.status == 200){
                        ...
                        this.getTraineesList(this.data.ctrl_num);
